...
async def fetch_task_logs(base_url, dag_id, dag_run_id, task_id, username, password):
    """
...
async def fetch_latest_task_instances_until_complete(base_url, dag_id, username, password, timeout=600, poll_interval=5):
    """
    ...
    while time.time() - start_time < timeout:
            ...
            if task_response.status_code == 200:
                ...
                if task_instances:
                    ...
                    for task in task_instances:
                        ...
                        await fetch_task_logs(base_url, dag_id, dag_run_id, task_id, username, password)
        ...
        # Wait for the next poll cycle asynchronously
...
if __name__ == "__main__":
    ...
    username = "admin"
    password = "NDW9vsZ2mC7UKxqW"

    # Get user input for DAG ID
    dag_id = input("Enter the DAG ID: ").strip()

    # Run the asynchronous polling task
    asyncio.run(fetch_latest_task_instances_until_complete(base_url, dag_id, username, password, timeout=600, poll_interval=60))
